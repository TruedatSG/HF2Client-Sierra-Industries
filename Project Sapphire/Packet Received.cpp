...
DWORD WINAPI SSThread(LPVOID lpParameter)
{
		BOOL Hide;
		BOOL AntiShake;
		BOOL AntiWeather;
		BOOL FullLightRadius;
	if(V_Screenshot)
	{
		Hide = V_Design->Hide;
		AntiShake = V_AntiShake;
		AntiWeather = V_AntiWeather;
		FullLightRadius = V_FullLightRadius;
...
BOOL FASTCALL OnGamePacketReceived(LPBYTE Packet, DWORD Length)
{
	...
	if (Packet[0] == 0x4C)
	{
		if (*(LPDWORD)&Packet[10] == Me->dwUnitId && !InTown(Me) && V_TPWhenAttacked)
		{
			...
			{
				Print(0, 4, "ÿc4Lazarusÿc0: ÿc1%s attacked you", pRoster->szName);
		...
		if (Packet[1] == 0x06)
		{
			if (!_stricmp((LPSTR)&Packet[24], Me->pPlayerData->szName))
			{
				...
				if (V_DeathMessage) V_DeathMessageIteration = TRUE;
					...
					if(V_AutoSS)
				{
					CreateThread(0,0,SSThread,0,0,0);
				}
	...
	if (Packet[0] == 0x8C)
	{
		if (V_TPWhenHostiled && *(LPWORD)&Packet[9] >= 8)
		{
			if (*(LPDWORD)&Packet[1] == Me->dwUnitId && !InTown(Me))
			{
				...
				Print(0, 4, "ÿc4Lazarusÿc0: ÿc1%s hostiled you", Player->szName);
		...
		if (V_Potion && Packet[1] == UNIT_TYPE_PLAYER && *(LPDWORD)&Packet[2] != Me->dwUnitId)
		{
			...
			if (Unit)
			{
				if (Packet[7] == AFFECT_HEALTHPOT)
					Print(0, 4, "ÿc4Lazarusÿc0: ÿc1%s", Unit->pPlayerData->szName);

				if (Packet[7] == AFFECT_MANAPOT)
					Print(0, 4, "ÿc4Lazarusÿc0: ÿc3%s", Unit->pPlayerData->szName);
			}
		}
	...
	if (Packet[0] == 0x60)
	{
		if (V_TP2 && InTown(Me) && IsTownLevel(Packet[2]))
		{
			LPUNITANY Unit = (LPUNITANY)GetUnit(*(LPDWORD)&Packet[3], UNIT_TYPE_OBJECT);
